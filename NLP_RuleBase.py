...
class NLP_RuleBase():
    '''
    sentence是使用者輸入的斷詞結果，
    self.terms 是Rule的關鍵詞的列表，
    透過雙層迴圈讓詞袋中每一個詞和關鍵詞表中的每一個關鍵字計算相似度，
    最後迴傳最優的分數、自己的 ID 與匹配到的字詞。
    RuleBase的工作也沒什麼，就負責把一堆Rule從json檔中讀入，並提供了一個對外的匹配接口:

    儲存規則的資料結構是字典，鍵為Rule.domain，值則是Rule的實例
    '''
    ...
    def match(self, sentence, threshold=0):
        ...
        Returns:
            ...
            sentence = 使用者輸入的斷詞結果
            terms = Rule的關鍵詞的列表，
            透過雙層迴圈讓詞袋中每一個詞和關鍵詞表中的每一個關鍵字計算相似度，最後迴傳最優的分數、自己的 ID 與匹配到的字詞。
        """
    ...
    def Load_Rules_Old_Format(self, path):
        ...
        with open(path, 'r', encoding='utf-8') as input:
            for line in input:
                ...
                new_rule = NLP_RuleBase(self.Rule_Amount(), rule_terms[0].split(','), self.model)
        ...
        with open(path, 'r', encoding='utf-8') as input:
            ...
            for data in json_data:
                ...
                if domain not in self.rules:
                    rule = NLP_RuleBase(domain, concepts_list, children_list, response, self.model)
